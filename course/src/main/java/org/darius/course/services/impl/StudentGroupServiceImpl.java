...
public class StudentGroupServiceImpl implements StudentGroupService {
    ...
    public StudentGroupResponse updateStudents(Long id, UpdateGroupStudentsRequest request) {
        ...
        if ("ADD".equalsIgnoreCase(request.getAction())) {
            ...
            throw new InvalidOperationException("Action invalide : " + request.getAction() + " (ADD ou REMOVE)");
        }
    ...
    public StudentGroupResponse findOrCreatePromoGroup(Long levelId, Long filiereId, Long semesterId) {
        ...
        // Chercher le groupe PROMO existant
        return studentGroupRepository
                .findBySemester_IdAndFiliereIdAndLevelIdAndType(
                        semesterId, filiereId, levelId, GroupType.PROMO
                )
                .map(mapper::toStudentGroupResponse)
                .orElseGet(() -> {
                    // Créer un nouveau groupe PROMO
                    StudentGroup newGroup = StudentGroup.builder()
                            ...
                            .filiereId(filiereId)
                            .semester(semester)
                            .maxSize(200) // capacité par défaut pour une promo
                            .studentIds(new ArrayList<>())
                            .build();
                    ...
                    return mapper.toStudentGroupResponse(
